...
if LOG_FORMAT != "json":
    print(f"""
==========================================================
  GabomaGPT v{VERSION} - IA Souveraine du Gabon
...
# GabomaGPT — Gestionnaire d'erreurs culturel gabonais
try:
    from open_webui.core.error_handler import (
    ...
    app.add_exception_handler(_RVE, validation_exception_handler)
    app.add_exception_handler(Exception, generic_exception_handler)
except ImportError:
    pass

# For GabomaGPT OIDC/OAuth2
...
# GabomaGPT — Payment router
try:
    from open_webui.routers.payment import router as payment_router
    app.include_router(payment_router, tags=["payment"])
except ImportError:
    pass  # Module httpx optionnel
...
# ── GabomaGPT — Health endpoint ──────────────────────────
@app.get("/health")
async def health_check():
    return {"status": "ok", "service": "gabomagpt-api"}


# ── GabomaGPT — Internal credit-tokens (appelé par Go service) ──
@app.post("/internal/credit-tokens")
async def internal_credit_tokens(request: Request):
    internal_secret = os.environ.get("INTERNAL_API_SECRET", "")
    req_secret = request.headers.get("X-Internal-Secret", "")
    if internal_secret and req_secret != internal_secret:
        raise HTTPException(status_code=403, detail="Accès refusé")
    body = await request.json()
    user_id = body.get("userId", "")
    plan = body.get("plan", "")
    tokens = body.get("tokens", 0)
    # En Phase 1, on log seulement — le crédit réel se fait côté client via polling
    logging.getLogger("gabomagpt").info(
        f"Credit tokens: user={user_id} plan={plan} tokens={tokens}"
    )
    return {"status": "ok", "credited": tokens}
...
async def generate_messages(
    ...
    All models configured in GabomaGPT are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/gabomagpt/gabomagpt/releases/latest",
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for GabomaGPT.
