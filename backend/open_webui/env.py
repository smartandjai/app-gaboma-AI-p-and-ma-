...
# OPEN_WEBUI_DIR should be the directory where env.py resides (gabomagpt/)
OPEN_WEBUI_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of OPEN_WEBUI_DIR (backend/)
BACKEND_DIR = OPEN_WEBUI_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (gabomagpt-dev/)
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "GabomaGPT")
if WEBUI_NAME != "GabomaGPT":
    WEBUI_NAME += " (GabomaGPT)"

WEBUI_FAVICON_URL = "https://gabomagpt.com/favicon.png"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("gabomagpt")}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data("gabomagpt", "CHANGELOG.md") or b"").decode()
...
if FROM_INIT_PY:
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "gabomagpt_data", "zip", DATA_DIR)
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "gabomagpt")
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "gabomagpt")
